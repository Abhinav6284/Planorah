...
const ProgressChartWidget = ({ data = [] }) => {
    ...
    return (
        <div className="relative h-full overflow-hidden rounded-[20px] border border-white/45 bg-white/58 p-5 shadow-[0_18px_44px_-30px_rgba(37,99,235,0.42)] backdrop-blur-xl dark:border-white/15 dark:bg-white/[0.06]">
            <motion.div
                className="pointer-events-none absolute -right-16 -top-16 h-52 w-52 rounded-full bg-blue-300/24 blur-3xl dark:bg-blue-500/14"
                animate={{ opacity: [0.18, 0.3, 0.18], scale: [0.99, 1.03, 0.99] }}
            ...
            <div className="relative z-10">
                <div className="mb-5 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        <div className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-r from-blue-600 to-sky-500 text-white shadow-[0_10px_20px_-12px_rgba(37,99,235,0.68)]">
                ...
                <div className="flex min-h-[120px] items-end gap-2.5">
                    {chartData.map((item, index) => {
                        ...
                        return (
                            <motion.div key={index} whileHover={{ y: -1.5 }} className="group flex flex-1 flex-col items-center gap-2">
                                <div className="relative flex h-full w-full items-end justify-center">
                                    ...
                                    <motion.div
                                        ...
                                        className={`w-full max-w-[34px] rounded-lg border transition-all duration-300 ${item.isToday
                                                ? 'border-orange-300 bg-gradient-to-t from-orange-500 to-amber-400 shadow-[0_10px_20px_-12px_rgba(249,115,22,0.68)]'
                                            : item.count > 0
                                                ? 'border-blue-300 bg-gradient-to-t from-blue-600 to-sky-500 shadow-[0_10px_20px_-12px_rgba(37,99,235,0.68)]'
                ...
                <div className="mt-4 flex items-center justify-center gap-5 border-t border-white/55 pt-3 text-xs dark:border-white/12">
                    <div className="flex items-center gap-1.5">
                        <span className="h-2.5 w-2.5 rounded-full bg-gradient-to-r from-blue-600 to-sky-500" />
