...
const DateTasksWidget = ({ tasks = [], variant = 'default' }) => {
    ...
    return (
        <div className={`bg-transparent px-2 py-3 h-full flex items-center gap-4 ${isExecution ? 'text-cyan-50' : ''}`}>
            {/* Date Circle */}
            <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${isExecution ? 'border-cyan-300/25 bg-black/10' : 'border-gray-300 dark:border-gray-600'}`}>
            ...
            {/* Divider */}
            <div className={`h-8 w-px ${isExecution ? 'bg-cyan-300/20' : 'bg-gray-300 dark:bg-gray-600'}`} />

            {/* Calendar Button */}
            <Link
                to="/scheduler"
                className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl border flex items-center justify-center transition-all hover:scale-105 flex-shrink-0 ${isExecution ? 'border-cyan-300/25 bg-[#091723] hover:bg-cyan-500/10' : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
            >
