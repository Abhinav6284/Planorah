...
export default function TaskList() {
    ...
    const autoGeneratedRoadmapsRef = useRef(new Set());
    ...
    const generateTasksForRoadmap = useCallback(async (roadmapId, options = {}) => {
        const { auto = false } = options;
        if (!roadmapId || roadmapId === 'all' || generatingTasks) return;
        ...
        try {
            await roadmapService.generateRoadmapTasks(Number(roadmapId));
            await fetchTasks();
        } catch (error) {
            console.error('Failed to generate roadmap tasks:', error);
            const message = error?.response?.data?.error || 'Failed to generate tasks for this roadmap.';
            if (!auto) {
    ...
    }, [fetchTasks, generatingTasks]);

    useEffect(() => {
        if (loading || generatingTasks) return;
        if (selectedRoadmap === 'all') return;
        if (tasks.length > 0) return;
        if (autoGeneratedRoadmapsRef.current.has(selectedRoadmap)) return;

        autoGeneratedRoadmapsRef.current.add(selectedRoadmap);
        generateTasksForRoadmap(selectedRoadmap, { auto: true });
    }, [loading, generatingTasks, selectedRoadmap, tasks.length, generateTasksForRoadmap]);
                    ...
                    {Object.keys(groupedTasks).length === 0 ? (
                        <div className="text-center py-20 bg-white dark:bg-charcoal rounded-2xl border border-gray-100 dark:border-charcoalMuted">
                            ...
                            <div className="flex flex-wrap items-center justify-center gap-3">
                                {isRoadmapScoped && (
                                    <button
                                        onClick={() => generateTasksForRoadmap(selectedRoadmap)}
                                        disabled={generatingTasks}
                                        className={`px-5 py-2.5 rounded-xl font-medium transition-colors ${generatingTasks
                                            ? 'bg-gray-300 text-gray-600 cursor-not-allowed'
                                            : 'bg-emerald-600 text-white hover:bg-emerald-500'
                                        }`}
                                    >
                                        {generatingTasks ? 'Generating Tasks...' : 'Generate Tasks for This Roadmap'}
                                    </button>
                                )}
                                {hasStatusFilter && (
                                    <button
